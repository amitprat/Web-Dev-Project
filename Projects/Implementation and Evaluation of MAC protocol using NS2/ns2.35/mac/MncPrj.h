...
private:
	...
	Handler * 	txHandler_;
	MncPrjWaitTimer *waitTimer;
	MncPrjSendTimer *sendTimer;
	MncPrjRecvTimer *recvTimer;
	int busy_ ;
	//edited
	int repeatnum;
	double send_time[MAX_REPEAT_COUNT];
	int count;
};
...
class MncPrjWaitTimer: public MncPrjTimer {
public: MncPrjWaitTimer(MncPrj *m) : MncPrjTimer(m) {}
	void handle(Event *e);
};
