...
process_exit (void)
{
  ...
  /* Destroy the current process's page directory and switch back
	...
	pd = cur->pagedir;
	free_all_resources(pd); //free all resources
	if (pd != NULL) 
    {
...
bool load_page_data(suppl_page_table *spage);
static bool load_segment (struct file *file, off_t ofs, uint8_t *upage,
              uint32_t read_bytes, uint32_t zero_bytes, bool writable) 
{
	...
	while (read_bytes > 0 || zero_bytes > 0) 
    {
	  ...
	  suppl_page_table *spage= update_suppl_page_table(NULL,ofs,upage,page_read_bytes, page_zero_bytes, writable);	
	  spage->ofs = ofs;
	  spage->addr = upage;
	  spage->read_bytes = page_read_bytes;
	  spage->zero_bytes = page_zero_bytes;
...
bool load_page_data(suppl_page_table *spage)
{
	uint8_t *kpage = frame_alloc_and_lock(spage);//palloc_get_page (PAL_USER);
	struct thread *t = thread_current ();
	uint32_t *pd = t->pagedir;
    if (kpage == NULL)
	{
    ...
    /* Load this page. */
	lock_acquire(&file_lock);
	struct file * mfile = file_reopen(spage->file);
	file_seek(mfile,spage->ofs);
    if (file_read((struct file *)mfile, kpage, spage->read_bytes) != (int) spage->read_bytes)
    {
		lock_release(&file_lock);
		frame_free(kpage);
        return false; 
	...
	lock_release(&file_lock);
    memset (kpage + spage->read_bytes, 0, spage->zero_bytes);
    /* Add the page to the process's address space. */
    if (!install_page (spage->addr, kpage, spage->writable)) 
    {
		printf("fail4");
		frame_free(kpage);
        return false; 
...
bool writeback_frame_data_to_file(suppl_page_table *spage,void *addr)
{
	struct thread *t = thread_current ();
	uint32_t *pd = t->pagedir;
	lock_acquire(&file_lock);
	struct file * mfile = file_reopen(spage->file);
	file_seek(mfile,spage->ofs);
    if((file_write((struct file *)mfile, addr, spage->read_bytes))!= (int)spage->read_bytes)
    {
		lock_release(&file_lock);
		printf("hello");
        return false; 
...
setup_stack (const char *cmd_line, void **esp) 
{
    ...
    uint32_t *pd;
    pd = cur->pagedir;
	suppl_page_table *spage= update_suppl_page_table(NULL,0,upage,0, 0, true);
	uint8_t *kpage = frame_alloc_and_lock(spage);
	if (kpage != NULL) 
    {
      ...
      if (install_page (upage, kpage, true))
	  {
		...
		spage->esp = *esp;
		spage->stack_pgs = 1;
		//overwrite_existing_page_entry(spage);
		//update  stack pointer for this page
		 #if TRACE_DEBUG
		printf("setup_stack-->:%p,%p",upage,kpage);
		#endif
	  }
    }
...
bool extend_stack (uint32_t *pd,uint32_t total_pages) 
{
	uint32_t *upage = ((uint8_t *) PHYS_BASE) - (total_pages)*PGSIZE;
	suppl_page_table *s_page = update_suppl_page_table(NULL,0,upage,0, 0, true);
	uint8_t *kpage = frame_alloc_and_lock(s_page);
	#if TRACE_DEBUG
	printf("returned");
	#endif
	int i=0;
    ...
    {
		 #if TRACE_DEBUG
		printf("i am getting called,:%d",total_pages);
		#endif
		s_page->esp = esp;
		s_page->stack_pgs = total_pages;
		if(install_page (upage, kpage, true)) 
		{
			 #if TRACE_DEBUG
			 printf("success");
